...
INIT_DIST = 5.0    # Initial distance between the cars

# Initial velocity
V_INIT = 90.0 / 3.6 # 100km/h -> 100/3.6 m/s

# Prediction horizon
PREDICTION_HORIZON = 30

# Constriants
SAFETY_DISTANCE = 0.5 + LENGTH
...
def mpc(veh_states, xref, split_car, last_ref_in_mpc, split_distance, drag_or_not, hard_split, x_last = None):
    """
    ...
    for k in range(PREDICTION_HORIZON):
        if type(x_last) != type(None) and drag_or_not == "1":
...
def cost_matrices(split_car, part):
    ...
    if part == 'velocity_heavy':
        d_weight = 1
        v_weight = 10
    if part == 'distance_heavy':
        d_weight = 10
        v_weight = 1

    # Cost matrices
    R = 5.0 * sparse.eye(NUM_CARS-1)
    ...
    for i in range(NUM_CARS):
        ...
        if i != NUM_CARS-1:
            q_vec[i] = d_weight*2.0     #distance cost

        q_vec[i+NUM_CARS-1] = v_weight*2.0      #velocity cost
        
        if i+1 == split_car and i+2 == split_car:
            q_vec[i] = d_weight*4.0   #distance cost for split car
            q_vec[i+NUM_CARS-1] = v_weight*4.0      #velocity cost for split car
...
def deltax_velocity_dependence(x_pred,i):
    R = -K * x_pred[NUM_CARS+i] * (1 - 13.41/(23.53 + x_pred[i])) # i+1 because when it is been sending to this func. the input is i e.g i=0 but we then need for the second vehicle which is i=1 actualy.
    S = -K/2 * (x_pred[NUM_CARS+i] ** 2) * (13.41/((23.53 + x_pred[i]) ** 2))
    Q = -K/2 * (x_pred[NUM_CARS+i]**2) * (x_pred[i] * (13.41/((23.53 + x_pred[i]) ** 2)) - (1 - 13.41/(23.53 + x_pred[i])))
    return R, S, Q
...
def animation(inital_veh_states, split_event, initial_xref):
    ...
    while MAX_TIME >= time:
        ...
        if (time + DT*PREDICTION_HORIZON) >= split_ready and try_split == True:
            ...
            if time >= split_ready - 2*DT: # If time is bigger than 1 time steps
                hard_split = False
            if time >= split_ready - 5*DT: # Change the ref 2 time steps before
        ...
        next_control_signals, x_last = mpc(veh_states, xref, split_car, last_ref_in_mpc, split_distance, drag_or_not, hard_split, x_last)
        ...
        time += DT
        dt_list.append(time)
        # Breakes animation after (the split in complete or is cancelled) and the system return to a stable position 
        
        if veh_states[0].x >= ROAD_LENGTH*5: #time >= split_ready and program_is_done(u_list):
            break
...
def program_is_done(u_list):
    ...
    for i in range(NUM_CARS-1):
        for j in range(4):
            summ += abs(u_list[-i*NUM_CARS-1-j])
    if summ/NUM_CARS <= 0.1:
        done = True
    #print(summ/NUM_CARS)
    return done
...
def split_event_finder():
    ...
    split_event.append(input_control("Split end position give in time: ", [5,2*DT*PREDICTION_HORIZON]))
