...
def update_states(veh_states, states, control_signals):
    ...
    for i in range(NUM_CARS):
        if i != 0:
            CD_distance = CD * (1 - (13.41 / (23.53 + states[i-1].deltax)))
...
def mpc(states, xref, split_car, last_ref_in_mpc, split_distance):
    """
    ...
    for k in range(PREDICTION_HORIZON):
        ...
        for i in range(NUM_CARS-1):                                         # The for loop is for defining safety distance for all cars
            ...
            if k >= (PREDICTION_HORIZON - last_ref_in_mpc):
                constraints += [[split_distance + LENGTH] <= x[2*(split_car-1),k]]  # -INTERVAL finns med för att skapa ett intervall
...
def animation(inital_veh_states, initial_states, split_event, initial_xref):
    ...
    while 5*MAX_TIME >= time:
        ...
        next_control_signals = mpc(states, xref, split_car, last_ref_in_mpc, split_distance)
